...
public class FeedListViewModel {
    ...
    public var morningOn = false
    public var eveningOn = false
    
    public init(morningOn: Bool = false, eveningOn: Bool = false) {
    ...
    @MainActor
    public func updateFeed(pet: Pet?, type: FeedSelection) {
    ...
    @MainActor
    public func getLatestFeed(pet: Pet?) async {
        let todaysFeed = todaysFeeds(pet: pet)
